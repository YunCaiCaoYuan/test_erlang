...
start_link(Host, Port, User, Password, Database, LogFun, Encoding, PoolId) ->
    ...
    Pid = spawn_opt(fun () ->
			     init(Host, Port, User, Password, Database,
				  LogFun, Encoding, PoolId, ConnPid)
		     end, [link, {min_heap_size, 20480}, {fullsweep_after, 20}]),
    post_start(Pid, LogFun).
...
do_query(Sock, RecvPid, LogFun, Query, Version) ->
	...
	ok ->
	    get_query_response(LogFun,RecvPid,
				    Version);
    {error, closed} ->
        case get(already_connected) of
        true -> exit(kill); %% 解决socket关闭后连接进程却还活着的问题
        _ -> 
            Msg = io_lib:format("Failed sending data on socket : ~p", [closed]),
	        {error, Msg}
        end;
	{error, Reason} ->
...
get_query_response(LogFun, RecvPid, Version) ->
	...
	{ok, <<Fieldcount:8, Rest/binary>>, _} ->
		...
		0 ->
        	...
        	{InsertId, _Rest2} = length_coded_binary(Rest1),    %% _Rest2里面还有返回其他信息的暂时丢掉
...
%% 这段从emysql的emysql_util搞过来的
length_coded_binary(<<>>) -> {0, <<>>};
length_coded_binary(<<FirstByte:8, Tail/binary>>) ->
    ...
    FirstByte == 254 ->
        ...
        {Word, Tail1};
    true -> exit(poorly_formatted_length_encoded_binary)
	end.
%%--------------------------------------------------------------------
