...
-define(MYSQL_DEFAULT_TIMEOUT, 10000).
...
fetch(Query) ->
    %%io:format("Date:~w Query: ~s~n", [erlang:localtime(), Query]),
...
handle_info({'DOWN', _MonitorRef, process, Pid, Info}, State) ->
	...
	{ok, Conn, NewState} ->
		case gb_trees:size(State#state.pids_pools) of
		0 -> global_event_listener:on_db_down();
		_ -> ignore
		end,
...
call_server(Msg, Timeout) ->
    if Timeout == undefined ->
	    gen_server:call(?SERVER, Msg, ?MYSQL_DEFAULT_TIMEOUT);
       true ->
